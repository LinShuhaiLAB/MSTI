...
tsne_plot<- DimPlot(seurat, reduction = "tsne", label = TRUE, cols = color,
                    group.by = "LouvainBayesian")
tsne_plot
ggsave(
  filename = "tsne.pdf",  
...
####--------------------part4:metabolite_pathway_analysis_in_cluster----------------
...
####--------------------part6:correlation PSAP with Sphingolipid_metabolism--------------
...
####--------------------part8:metabolite pathways in different state------------------
...
####--------------------part9:Sphingolipid metabolism in state4 and state5---------------
...
####--------------------part10:CNV in state4 and state5 -----------------------
rownames(data_test) <- data_test$barcodes
result_state <- merge(state_cluster2,data_test,by = 0,all.x = TRUE)
result_state2 <- result_state[,c(2,28)]
result_long_state<- result_state2 %>%
  pivot_longer(cols = c(mean), names_to = "variable", values_to = "mean")
colnames(result_long_state) <- c("state_cluster","variable","mean")
result_long_state$state_cluster <- factor(result_long_state$state_cluster, 
                                          levels = c("4","5"))
result_long_state$state_cluster <- case_when(
  result_long_state$state_cluster == 4 ~ "cell fate 1",
  result_long_state$state_cluster == 5 ~ "cell fate 2"
)
comparisons <- list(
  c("cell fate 1", "cell fate 2")
)
CNV_result_state <- ggplot(result_long_state, aes(x = state_cluster, y = mean, 
                                                  fill = state_cluster)) +
  geom_violin() +
  geom_boxplot(width = 0.2 ,fill = "white",outliers = FALSE)+
  scale_y_continuous(limits = c(min(result_state2$mean), max(result_state2$mean)))+
  scale_fill_manual(values=c("#D9A896","#A77852"))+
  labs(x = "", y = "CNV result") +
  theme_classic()+
  stat_compare_means(
...
CNV_result_state
ggsave(
  filename = "CNV_result2.pdf",  
  plot = CNV_result_state,                
  width = 1010/300,                     
  height = 850/300,                      
  units = "in"                    
)
####--------------------part11:RCTD Analysis ------------------------------
